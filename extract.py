...
def extract_keep_out_zones(gerber, aperature_diameter=0.1):
    ...
    Returns:
        rectangles (tuple list): A list of tuples representing the rectangles of the keep-out zones. Each tuple contains four points in the order (top_left, top_right, bottom_right, bottom_left).
    """
    lines = [obj for obj in gerber.objects if isinstance(obj, Line) and 
             isinstance(obj.aperture, CircleAperture) and 
             abs(obj.aperture.diameter - aperature_diameter) < 0.0001] # Floating point comparison, as Python can be strange sometimes

    rectangles = []
    used_indices = set()

    # Find a continuation line
    ...
    for index, line in enumerate(lines):
        ...
        for _ in range(3):  # Need four lines to close the loop
